...
import { SettlementReportWithCouriers, CourierReportViewModel } from '@/lib/types/settlement-report';
...
export default function SettlementReportPage({ params }: SettlementReportPageProps) {
  const router = useRouter();
  const [report, setReport] = useState<SettlementReportWithCouriers | null>(null);
  const [selectedCourierId, setSelectedCourierId] = useState<string | null>(null);
  const [courierDetails, setCourierDetails] = useState<CourierReportViewModel | null>(null);
  ...
  useEffect(() => {
    ...
    async function loadCourierDetails() {
      try {
        ...
        const data = await getCourierReportDetails(report.id, selectedCourierId);
  ...
  const renderSettlementDetails = (item: any) => {
    ...
    switch (item.settlement_type) {
      case 'kurly':
        return (
          <div className="mt-2 bg-gray-50 p-3 rounded-md">
            <h5 className="text-sm font-medium text-gray-900 mb-1">컬리 정산 세부 정보</h5>
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {details.details.kurly_settlements.map((detail: any) => (
                    <tr key={detail.id}>
                      <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-500">{detail.company_name}</td>
                      <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-500">{detail.center || '-'}</td>
                      <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-500">{detail.region || '-'}</td>
                      <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-500">{detail.amount?.toLocaleString() || '-'}</td>
                      <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-500">{detail.settlement_amount?.toLocaleString() || '-'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      case 'coupang':
        return (
          <div className="mt-2 bg-gray-50 p-3 rounded-md">
            <h5 className="text-sm font-medium text-gray-900 mb-1">쿠팡 정산 세부 정보</h5>
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-100">
                  <tr>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {details.details.coupang_settlements.map((detail: any) => (
                    <tr key={detail.id}>
                      <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-500">{detail.courier_name}</td>
                      <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-500">{detail.delivery_count?.toLocaleString() || '-'}</td>
                      <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-500">{detail.unit_price?.toLocaleString() || '-'}</td>
                      <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-500">{detail.supply_price?.toLocaleString() || '-'}</td>
                      <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-500">{detail.vat?.toLocaleString() || '-'}</td>
                      <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-500">{detail.total_amount?.toLocaleString() || '-'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      case 'general':
        ...
        if (!generalDetails || generalDetails.length === 0) {
          return <div className="mt-2 bg-gray-50 p-3 rounded-md">기타 정산 내역이 없습니다.</div>;
        }
        ...
        return (
          <div className="mt-2 bg-gray-50 p-3 rounded-md">
            <h5 className="text-sm font-medium text-gray-900 mb-1">기타 정산 세부 정보</h5>
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-100">
                  <tr>
      ...
      default:
        return <div className="mt-2 p-3 bg-gray-50 rounded-md">상세 정보를 찾을 수 없습니다.</div>;
    }
  };
    ...
    <div className="py-6">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
        <div className="flex justify-between items-center">
          <h1 className="text-2xl font-semibold text-gray-900">정산 보고서 상세</h1>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
        <div className="py-4">
          ...
          {isLoading ? (
            ...
            <div className="text-center py-10">
              <h3 className="mt-2 text-lg font-medium text-gray-900">정산 보고서를 찾을 수 없습니다</h3>
              <p className="mt-1 text-sm text-gray-500">요청하신 보고서가 존재하지 않거나 접근할 수 없습니다.</p>
              <div className="mt-6">
                <button
                  type="button"
                  onClick={handleBack}
            ...
            <div className="space-y-6">
                ...
                <div className="border-t border-gray-200 px-4 py-5 sm:px-6">
                  <dl className="grid grid-cols-1 gap-x-4 gap-y-8 sm:grid-cols-2">
                    <div className="sm:col-span-1">
                      <dt className="text-sm font-medium text-gray-500">기사 수</dt>
                      <dd className="mt-1 text-sm text-gray-900">{report.couriers.length}명</dd>
                    </div>
                    <div className="sm:col-span-1">
                      <dt className="text-sm font-medium text-gray-500">총액</dt>
                      <dd className="mt-1 text-sm text-gray-900">
                        {report.couriers.reduce((sum, courier) => sum + parseFloat(courier.total_amount.toString()), 0).toLocaleString()}원
                      </dd>
                    </div>
                    <div className="sm:col-span-1">
                      <dt className="text-sm font-medium text-gray-500">생성 일시</dt>
                      <dd className="mt-1 text-sm text-gray-900">
              ...
              <div className="bg-white shadow sm:rounded-lg">
                <div className="px-4 py-5 sm:px-6">
                  <h3 className="text-lg leading-6 font-medium text-gray-900">기사별 정산 내역</h3>
                </div>
                  ...
                  <div className="grid grid-cols-6 gap-4">
                    <div className="col-span-2 border-r border-gray-200">
                      <div className="px-4 py-5">
                        <h4 className="text-sm font-medium text-gray-500 mb-4">기사 목록</h4>
                            ...
                            <li key={courier.id}>
                              <a
                                ...
                                className={`block px-4 py-4 hover:bg-gray-50 ${
                                  selectedCourierId === courier.courier_id ? 'bg-blue-50' : ''
                                }`}
                              >
                                <div className="flex items-center justify-between">
                                  <p className="text-sm font-medium text-gray-900">{courier.courier_name}</p>
                                  <p className="text-sm font-medium text-blue-600">{parseFloat(courier.total_amount.toString()).toLocaleString()}원</p>
                                </div>
                              </a>
                            </li>
                          ))}
                      ...
                      <div className="px-4 py-5">
                        {isLoadingDetails ? (
                          <div className="py-10 text-center">
                            <div className="inline-block h-8 w-8 animate-spin rounded-full border-4 border-solid border-blue-600 border-r-transparent"></div>
                            <p className="mt-2 text-gray-500">기사의 정산 내역을 불러오는 중...</p>
                          </div>
                        ) : !courierDetails ? (
                          <div className="py-10 text-center">
                            <p className="text-gray-500">기사를 선택하여 상세 내역을 확인하세요.</p>
                          </div>
                        ) : (
                          <div>
                            <div className="flex justify-between items-center mb-4">
                              <h4 className="text-lg font-medium text-gray-900">{courierDetails.courier.courier_name} 정산 내역</h4>
                              <span className="text-lg font-medium text-blue-600">{parseFloat(courierDetails.courier.total_amount.toString()).toLocaleString()}원</span>
                            </div>
                                ...
                                {courierDetails.items.map((item) => (
                                  <div key={item.id} className="border rounded-lg p-4">
                                    <div className="flex justify-between items-center">
                                      ...
                                      <p className="text-sm font-medium text-blue-600">{parseFloat(item.amount.toString()).toLocaleString()}원</p>
                                    </div>
