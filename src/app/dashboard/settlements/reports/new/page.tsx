...
export default function CreateSettlementReportPage() {
  ...
  const [selectedCouriersData, setSelectedCouriersData] = useState<
    Record<string, any[]>
  >({});
  ...
  // 선택된 기사들의 정산 내역 로드
  useEffect(() => {
    if (step !== 2 || !startDate || !endDate || selectedCouriers.length === 0)
      return;
      ...
      for (const courierId in selectedCouriersData) {
        ...
        for (const settlement of settlements) {
          ...
          switch (settlement.settlement.settlement_type) {
            case "kurly":
              {
                ...
                amount = kurlyDetails.reduce(
                  (sum, item) => sum + (item.settlement_amount || 0),
                  0
                );
              }
              break;
            ...
            case "general":
              {
                const generalDetails = settlement.details as {
                  columns: string[];
                  rows: any[];
                };
                const amountIndex = generalDetails.columns.findIndex(
                  (col) => col === "금액"
                );
                ...
                if (amountIndex >= 0) {
                  amount = generalDetails.rows.reduce((sum, row) => {
                    const amountValue =
                      row[generalDetails.columns[amountIndex]];
          ...
          if (Array.isArray(settlement.details)) {
            itemId =
              settlement.details.length > 0 ? settlement.details[0].id : "";
          } else if (
            settlement.details.rows &&
            settlement.details.rows.length > 0
          ) {
                                ...
                                <tbody className="bg-white divide-y divide-gray-200">
                                  {settlements.map((settlement) => {
                                    ...
                                    if (Array.isArray(details)) {
                                      ...
                                      const amountIndex =
                                        details.columns.findIndex(
                                          (col: string) => col === "금액"
                                        );
                                      if (amountIndex >= 0) {
                                        amount = details.rows.reduce(
                                          (sum: number, row: any) => {
                                            const amountValue =
                                              row[details.columns[amountIndex]];
                                            return (
                                              sum +
